        ...
        >
            ...
            {explanation.clinicalSummary && (
                <motion.div variants={fadeIn} className="glass-card p-6 result-section">
                    ...
                    <div className="result-text-block">
                        <p className="text-gray-300 leading-relaxed text-left">{explanation.clinicalSummary}</p>
                    </div>
                </motion.div>
            )}

            {/* Section 2: Critical Values */}
                        ...
                        {explanation.criticalValues.map((cv, i) => (
                            <div key={i} className="finding-card critical">
                                <div className="flex items-center justify-between mb-2">
                                    ...
                                    <span className={`badge ${cv.urgency === 'immediate' ? 'badge-danger' : 'badge-warning'
                                        }`}>
                                        {cv.urgency}
                                    </span>
                                </div>
                                ...
                                {explanation.abnormalFindings.map((af, i) => (
                                    <div key={i} className="finding-card warning">
                                        <div className="flex items-center justify-between mb-1">
                                            ...
                                            <span className={`badge ${af.trend === 'worsening' ? 'badge-danger'
                                                : af.trend === 'improving' ? 'badge-success'
                                                    : af.trend === 'new' ? 'badge-warning'
                                                        : 'badge-info'
                                                }`}>
                                                {af.trend}
                                            </span>
                        ...
                        {explanation.differentialDiagnosis.map((ddx, i) => (
                            <div key={i} className="finding-card">
                                <div className="flex items-center justify-between mb-2">
                                    ...
                                    <span className={`badge ${ddx.likelihood === 'high' ? 'badge-success'
                                        : ddx.likelihood === 'medium' ? 'badge-warning'
                                            : 'badge-info'
                                        }`}>
                                        {ddx.likelihood}
                                    </span>
                                ...
                                {ddx.supportingFindings?.length > 0 && (
                                    <div className="space-y-1">
                                        {ddx.supportingFindings.slice(0, 3).map((sf, j) => (
                                            <p key={j} className="text-gray-500 text-xs flex items-start gap-1.5">
            ...
            {explanation.suggestedNextSteps?.length > 0 && (
                <motion.div variants={fadeIn} className="glass-card p-6">
                  ...
                  ${step.timeframe === 'immediate' ? 'bg-red-500/20 text-red-400'
                                        : step.timeframe === 'days' ? 'bg-amber-500/20 text-amber-400'
                                ...
                                <div className="flex-1">
                                    <div className="flex items-center gap-2 mb-1">
                                        ...
                                        <span className={`badge text-xs ${step.timeframe === 'immediate' ? 'badge-danger'
                                            : step.timeframe === 'days' ? 'badge-warning'
                                                : 'badge-info'
                                            }`}>
                                            {step.timeframe}
                                        </span>
            ...
            <div className="grid md:grid-cols-2 gap-6">
                ...
                {explanation.clinicalPearls?.length > 0 && (
                    <motion.div variants={fadeIn} className="glass-card p-6 bg-gradient-to-br from-purple-500/5 to-transparent border-purple-500/20">
                        <h3 className="section-title text-purple-400">
                            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
