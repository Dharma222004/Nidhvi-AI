...
} = require("../services/hospitalFinderService");
const { translateReport } = require("../services/translationService");
const {
  getStandardDisclaimers,
  detectRedFlags,
...
IMPORTANT: Return ONLY valid JSON. Be medically accurate but patient-friendly.`;
      ...
      messages: [
        {
          role: "system",
          content:
            "You are a medical AI providing safe, accurate analysis. Always include appropriate disclaimers and avoid dangerous advice.",
        },
        {
          role: "user",
    ...
    const hospitalSearchParams = {
      condition:
        analysis.possibleConditions?.[0]?.condition || "general checkup",
      ...
      recommendation: {
        ...
        action:
          'Click "Find Hospitals & Doctors" below to find specialists in your area',
        timeline:
          analysis.severity === "critical"
            ? "Immediately"
            : analysis.severity === "high"
    ...
    console.log(
      `Doctor visit needed: ${hospitalSearchParams.needsDoctor}, Severity: ${hospitalSearchParams.severity}`,
    );
      ...
      messages: [
        {
          role: "system",
          content:
            "You are a helpful medical assistant. Provide safe, context-based answers to patient questions.",
        },
        {
          role: "user",
          content: prompt,
...
router.post("/find-hospitals", async (req, res) => {
  try {
    ...
    switch (severity) {
      case "critical":
        urgencyMessage =
          "🚨 URGENT: Please seek immediate medical attention. Visit the nearest emergency department.";
        break;
      case "high":
        urgencyMessage =
          "⚠️ HIGH PRIORITY: Schedule an appointment within 1-2 days.";
        break;
      case "medium":
        urgencyMessage =
          "📋 Schedule an appointment within 1-2 weeks for proper evaluation.";
        break;
      default:
        urgencyMessage =
          "✅ This can be discussed at your next routine checkup.";
    }
    ...
    if (error.message.includes("API key")) {
      userMessage =
        "Hospital search service is not configured. Please contact support.";
    } else if (
...
router.post("/translate", async (req, res) => {
  try {
    const { content, targetLanguage, analysisData = null } = req.body;
...
Note: This is an AI-generated analysis. Please consult a qualified healthcare professional for medical advice.
    ...
    console.log(
      `Translation successful to ${translationResult.targetLanguageName}`,
    );
...
router.post("/analyze-stream", upload.single("file"), async (req, res) => {
  ...
  try {
    ...
    send("status", {
      message: "Extracting text from your report...",
      progress: 10,
    });
...
Return JSON with these exact fields:
{
  ...
  "disclaimer": "Always consult a qualified healthcare professional."
      ...
      messages: [
        {
          role: "system",
          content:
            "You are a medical AI assistant. Return ONLY valid JSON, no markdown.",
        },
    ...
    const detectedLocationStr = detectedLoc
      ? detectedLoc.fullAddress ||
        detectedLoc.city ||
        (detectedLoc.pincode ? `Pincode ${detectedLoc.pincode}` : null)
      : null;
    ...
    send("status", {
      message: "Generating patient-friendly explanation...",
      progress: 55,
    });
    ...
    try {
      if (mode === "clinician") {
        ...
        send("explanation_final", {
          clinicalSummary: fullExplanation,
          mode: "clinician",
        });
      } else {
      ...
      send("explanation_final", {
        ...
        explanation:
          analysis.summary || "Analysis complete. Please consult your doctor.",
        nextSteps: analysis.nextSteps || [],
        questionsForDoctor: analysis.questionsForDoctor || [],
      });
    }
